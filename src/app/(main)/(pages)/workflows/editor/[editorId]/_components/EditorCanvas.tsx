...
import { EditorCardCanvasType, EditorNode } from "@/lib/types";
...
const EditorCanvas = (props: Props) => {
  ...
  const onDrop = useCallback(
    (e: any) => {
      e.preventDefault();
      const type: EditorCardCanvasType["type"] = e.dataTransfer.getData(
        "application/reactflow"
      );
      if (typeof type === "undefined" || !type) return;

      const triggerAlreadyPresent = state.Editor.elements.find(
        (el) => el.type === "Trigger"
      );

      if (type === "Trigger" && triggerAlreadyPresent) {
        toast({
          title: "Error setting trigger",
          description: "only 1 trigger can be added",
        });
        return;
      }
    ...
    <ResizablePanelGroup direction='horizontal' className=''>
      <ResizablePanel defaultSize={70}>
        <div className='flex h-full justify-center items-center'>
          <div className='w-full h-full pb-[7px] relative'>
            {isWorkFlowLoading ? (
